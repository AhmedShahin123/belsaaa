...
class UserRepository extends BaseRepository
{
     ...
     * @return mixed
     */
    public function getAdminPaginated($paged = 25, $orderBy = 'created_at', $sort = 'desc', $term = null) : LengthAwarePaginator
    {
        $query = $this->model
            ->with('roles', 'permissions', 'providers')
            ->emptyType('administrator')
            ->orderBy($orderBy, $sort);
        ...
        return DB::transaction(function () use ($data) {
            ...
            if ($user) {
                ...
                if (!isset($data['roles']) || ! count($data['roles'])) {
                    $data['roles'] = [$this->roleRepository->first()];
