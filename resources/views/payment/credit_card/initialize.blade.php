...
<form style="direction: rtl" id="form-container" method="post" action="/charge">
    <!-- Tap element will be here -->
    <div id="element-container"></div>
    <div id="error-handler" role="alert"></div>
    <div id="success" style=" display: none;;position: relative;float: left;">
        نجاح! الرمز الخاص بك هو <span id="token"></span>
    </div>
    <!-- Tap pay button -->
    <button id="tap-btn">إرسال</button>
</form>
@endsection
...
<script !src="">
    ...
    var tap = Tapjsli('{{config('tap_payments.publishable_api_key')}}');
    ...
    form.addEventListener('submit', function(event) {
        ...
        tap.createToken(card).then(function(result) {
            ...
            if (result.error) {
                ...
                handleSuccessToken(result.id);
            }
        });
    });

    function handleSuccessToken(token) {
        $form = $('<form method="POST" action="{{route("payment.credit_card.create", ["hash" => $hash])}}">');
        $form.append($('<input type="hidden" value="'+token+'" name="token">'));
        $form.append($('@csrf'));
